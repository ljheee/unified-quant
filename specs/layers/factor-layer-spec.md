...
| F15a | Upstream partition visibility violates factor run-visible cutoff. | 5 | Factor publication governance | `tests/test_release_acceptance.py::test_freshness_violation_rejects_publication` | Passed |
| F15b | Rejected quarantine artifacts cannot be read as accepted factor partitions. | 5 | Factor store quarantine boundary | `tests/test_release_acceptance.py::test_quarantine_is_not_an_accepted_factor_partition` | Passed |
| F15c | Engine request rejects sessions beyond visible cutoff. | 2A | Engine input selection | `tests/test_factor_engine_contract.py::test_future_session_fails_closed_before_planning` | Passed |
| F15d | Empty/all-null/publication intent produce governed statuses. | 2A | FactorResult semantics | `tests/test_factor_engine_contract.py::test_factor_result_empty_all_null_and_partial_semantics`; `test_publication_intent_rejects_empty_result` | Passed |
| F16 | Raw-price factors do not consume adjustment data. | 3 | Dependency isolation | `tests/test_raw_price_engine.py::test_raw_price_factors_do_not_consume_adjustment_data` | Passed |
| F17 | Pinned raw-price logical/artifact golden vectors match declared profile. | 4A | Serialization and environment profile | `tests/test_repro_staging.py::test_raw_price_golden_vectors_and_profile_contract` | Passed for covered locked environment |
| F18a | Configured null/coverage thresholds reject failed quality reports. | 5 | Configurable factor quality gates | `tests/test_release_acceptance.py::test_null_rate_threshold_rejects_publication`; `test_coverage_threshold_rejects_publication` | Passed |
| F18c | Accepted reader honors reviewed warning policy without accepting it for reject-all stores. | 5 | Quality policy binding/read path | `tests/test_release_acceptance.py::test_warning_policy_publishes_and_reads_bound_warning_report`; `test_warning_policy_allows_bound_warning_report` | Passed |
| F19b | Quarantine data/manifest tampering fails closed. | 5 | Quarantine governance | `tests/test_release_acceptance.py::test_quarantine_tampered_data_and_manifest_fail_closed` | Passed |
| F18d | Local threshold decisions and persisted quality report checks agree before publication. | 5 | Quality binding consistency | `tests/test_release_acceptance.py::test_local_and_bound_quality_checks_must_agree` | Passed |
